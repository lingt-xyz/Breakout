...
Game::Game(GLuint width, GLuint height)
    : State(GAME_MENU), Width(width), Height(height), Level(0), Lives(3)
{
...
void Game::Init()
{
    ...
    Shader shader_particle = ResourceManager::GetShader("particle");
    Particles = new ParticleGenerator(shader_particle, ResourceManager::GetTexture("particle"), 500);
    Shader shader_post_processing = ResourceManager::GetShader("postprocessing");
    Effects = new PostProcessor(shader_post_processing, this->Width, this->Height);
    Text = new TextRenderer(this->Width, this->Height);
    Text->Load("fonts/FreeMono.ttf", 24);
    // load levels
    for (int i = 0; i < 4; ++i)
    {
        GameLevel lvl;
...
void Game::ProcessInput(GLfloat dt)
{
    if (this->State == GAME_MENU)
    {
        if (Input::Keys[GLFW_KEY_ENTER] && !Input::KeysProcessed[GLFW_KEY_ENTER])
        {
            ...
            Input::KeysProcessed[GLFW_KEY_ENTER] = GL_TRUE;
        }
        if (Input::Keys[GLFW_KEY_W] && !Input::KeysProcessed[GLFW_KEY_W])
        {
            ...
            Input::Keys[GLFW_KEY_W] = GL_FALSE;// indicating we have finised processing
        }
        if (Input::Keys[GLFW_KEY_S] && !Input::KeysProcessed[GLFW_KEY_S])
        {
            if (this->Level > 1)
                --this->Level;
            Input::Keys[GLFW_KEY_S] = GL_FALSE;// indicating we have finised processing
        }
    }
...
void Game::Render()
{
    if (this->State == GAME_ACTIVE || this->State == GAME_MENU || this->State == GAME_WIN)
    {
        ...
        std::stringstream string_lives; string_lives << this->Lives;
...
{
    std::string path = "levels/level" + std::to_string(this->Level) + ".lvl";
    this->Levels[this->Level - 1].Load(path.c_str(), this->Width, this->Height * 0.5f);
...
void Game::ResetKey(){
    memset(Input::KeysProcessed, 0, sizeof(Input::KeysProcessed));
}
